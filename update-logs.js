const updateLogs = [
        ...
        "date": "2025-11-15",
        "content": [
            "<strong>【重磅功能】新版本推播通知，重要更新不再錯過！</strong>",
            "為了讓您總能第一時間體驗到 Astra 的最新功能與改進，我們正式推出了「新版本推播通知」系統。現在，您可以授權 Astra 在背景為您檢查更新，並在我們發布新版本時，主動發送一則系統通知提醒您。",
            "<strong>✨ 功能亮點：</strong>",
            "<ul><li><strong>及時提醒：</strong>一旦有新版本發布，您的裝置將會收到一則簡潔的通知，即使您沒有開啟 App。</li><li><strong>智慧檢查：</strong>此功能由應用程式在背景低耗電運行，每隔一段時間自動檢查，兼顧了時效性與裝置效能。</li><li><strong>由您掌控：</strong>我們極度重視您的體驗，通知功能預設為關閉。您需要手動前往「設定 > 關於 > 通知設定」中點擊按鈕來啟用它。</li></ul>",
            "<strong>【iOS 使用者請注意】啟用通知的特別說明：</strong>",
            "由於 iOS 系統的隱私設計，啟用通知需要您完成兩個簡單的步驟：",
            "<ol><li><strong>1. 將 Astra 加入主畫面：</strong>您必須先透過 Safari 瀏覽器的「分享」選單，將本站「加入主畫面」，使其成為一個桌面 App。</li><li><strong>2. 從桌面圖示啟動並授權：</strong>啟動您加入到主畫面的 Astra App，然後在設定頁面中點擊「啟用新版本推播通知」按鈕，並在跳出的系統視窗中選擇「允許」。</li></ol>",
            "我們相信，這項新功能將使您與 Astra 的每一次互動都保持在最新、最完美的狀態。感謝您的支持！<br><br>Astra 開發團隊 敬上"
        ]
    },
        {
    "version": "15.10.0",
